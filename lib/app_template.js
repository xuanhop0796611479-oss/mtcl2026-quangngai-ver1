...
}catch(e){ console.error('Loi tab mat song (khong anh huong tab khac):', e); }
...
(function(){
 ...
 }catch(e){ console.error('Loi tab Xa/phuong (khong anh huong tab khac):', e); }
})();
...
(function(){
  ...
  let tg='<tr><th>#</th><th class="l">eUTraCell Old</th><th class="l">eNodeB Old</th><th class="l">Phường/Xã</th><th>TRAFFIC_4G (MB)</th></tr>';
  if(GL.length===0) tg+='<tr><td colspan=5>Không có cell 4G lưu lượng = 0 (hoặc chưa lấy được dữ liệu).</td></tr>';
  GL.forEach((x,i)=>{tg+=`<tr><td>${i+1}</td><td class="l"><b>${x.cell}</b></td><td class="l">${x.enb||'-'}</td><td class="l">${x.xa||'-'}</td><td><span class="tag kd">0</span></td></tr>`;});
